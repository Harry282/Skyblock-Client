    ...
    fun hasUpdate(): Int {
        try {
            val client: CloseableHttpClient =
                HttpClients.custom().setUserAgent("SkyblockClient/${SkyblockClient.MOD_VERSION}").build()
            ...
            return DefaultArtifactVersion(latest.asString.replace("pre", "beta"))
                .compareTo(DefaultArtifactVersion(SkyblockClient.MOD_VERSION.replace("pre", "beta")))
        } catch (e: Exception) {
            e.printStackTrace()
        }
        return 0
